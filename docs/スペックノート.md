...
### 8.7.10 Key Hash
Key HashはDDS DataWriterによって登録されたobjectsの集合の内部で変更されるdata-objectを一意に識別するためのkeyのヒントを提供する。

通常、keyはserializeされたdata submessageの一部である。key hashを使用することで、受信したdata-objectからfull keyをdeserializeするよりも高速な代替手段を提供し実装の利便性が向上します。

DataReaderがkey hashを受信しなかった場合、それはdata自身から計算される。もし、submessageにdataが含まれない場合、 default zero-valued key hashがDataReaderによって使用される。

もしKey Hashが存在していれば9.6.3.3で説明されているように計算される。