...
定義により、Topicは1つのdata typeにのみ対応する。しかし、いくつかのtopicは同じdata typeを参照するかもしれない。したがって、Topicは単一のdata typeを識別し、そのtypeの単一のinstanceからinstanceの集合全体に至るまでのデータを識別します。これはFigure 2.3で仮定のdata-type"Foo"について示す。
