# SampleAnalysis
RustDDSを参考実装としてRTPSの解析を行ったときの**個人的な**メモ
たまにFastDDSも読む
...
## mio::poll
RustDDSでは、mioのv0.6という古いバージョンが使用されているため、
本実装では、新しいバージョンを使用することを検討したが、
mio v0.8のpollにmpscチャネルを登録する方法に困まり、v0.6を使用することにした。
