// RustDDS: examples shapes_demo Apache-2.0
...
use mio_extras::{channel, timer::Timer}; // pollable channel
use rand::prelude::*;
...
fn main() {
    ...
    poll.register(&timer, TIMTER, Ready::readable(), PollOpt::edge())
        .unwrap();

    let is_publisher = matches.get_flag("publisher");
    let is_subscriber = matches.get_flag("subscriber");

    // Don't allow coexistance of publisher and subscriber
    assert!(!(is_publisher && is_subscriber));
            ...
            Some(ref mut writer) => {
                ...
                if last_write + loop_delay < now {
                    writer
                        ...
                        .unwrap_or_else(|e| eprintln!("DataWriter write failed: {:?}", e));
                    eprintln!("Writing shape color {}", &color);
                    last_write = now;
                }
            }
            None => {
