...
impl EventLoop {
    pub fn new(
        ...
        // spdp_send_timer.set_timeout(Duration::new(2, 0), ()); // TODO:
            ...
            for event in events.iter() {
                match TokenDec::decode(event.token()) {
                    TokenDec::ReservedToken(token) => match token {
                        ...
                        ADD_WRITER_TOKEN => loop {
                            while let Ok(writer_ing) = self.add_writer_receiver.try_recv() {
                                eprintln!("@ev_loop: add writer: entity_id: {:?}", writer_ing.guid);
                        ...
                        ADD_READER_TOKEN => {
                            while let Ok(reader_ing) = self.add_reader_receiver.try_recv() {
                                eprintln!("@event_loop received add reader");
                        ...
                        DISCOVERY_SEND_TOKEN => {
                            todo!(); // send spdp msg
                                     // self.spdp_send_timer.set_timeout(Duration::new(2, 0), ());
                        }
